...
export const TileGrid: React.FC<Props> = ({ config }) => {
  ...
  const handleContinue = () => {
    const updatedAnswers = { ...state.answers, [config.id as string]: selected };
  ...
  return (
    <>
      ...
      {config.multiSelect && (
        <div className={`mt-4 sm:mt-6 w-full flex justify-center transition-opacity duration-300 ${selected.length > 0 ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}>
          <button
            ...
            className={`w-full max-w-sm py-3 sm:py-4 rounded-xl text-base sm:text-lg font-bold flex items-center justify-center gap-2 transition-all duration-300 ${isLight
